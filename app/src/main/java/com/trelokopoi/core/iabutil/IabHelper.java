...
package com.trelokopoi.core.iabutil;
...
import com.quizdom.App;
import com.quizdom.L;
import com.quizdom.LocalStorage;
import com.quizdom.WebApi;
...
public class IabHelper {
 ...
 // is set to true if dispose is called while a thread is running. Allows graceful shutdown
    ...
    public static boolean verifyDeveloperPayload(Purchase p) {
        ...
        L.debug("original payload:" + payload);
        L.debug("generated payload:" + generatePayload(p.getSku()));
        if (payload.equals(generatePayload(p.getSku()))) {        	
        	return true;
        }
    ...
    public static String generatePayload(String skuId) {
    	    	    	
        String payload = App.TAG;
        	   payload +=LocalStorage.getUsername();
        	   payload +=skuId;
        	   
        L.debug("payload value: "+payload);
 	    payload = WebApi.sha1Hash(payload);    	
 	    L.debug("payload sha1: "+payload);
